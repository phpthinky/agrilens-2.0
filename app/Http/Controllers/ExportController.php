...
class ExportController extends Controller
{
    ...
    public function export(Request $request): StreamedResponse
    {
        ...
        return response()->streamDownload(function () use ($samples, $sampleId) {
            ...
            $headers = [
                ...
                'fert_urea_kgha', 'fert_tsp_kgha', 'fert_mop_kgha',
            ...
            foreach ($samples as $s) {
                ...
                fputcsv($out, [
                    ...
                    $fr['urea_kgha'] ?? '', $fr['tsp_kgha'] ?? '', $fr['mop_kgha'] ?? '',
            ...
            foreach ([
                ...
                ['fert_urea_kgha', 'Urea (46-0-0) application rate', 'kg/ha'],
                ['fert_tsp_kgha', 'Triple Superphosphate (0-46-0) rate', 'kg/ha'],
                ['fert_mop_kgha', 'Muriate of Potash (0-0-60) rate', 'kg/ha'],
    ...
    // ── Single-sample formatted Excel export (mirrors fertilizer schedule) ──
    public function exportSample(SoilSample $sample): \Illuminate\Http\Response
    {
        $user = Auth::user();
        ...
        $fertRec = $nStatus = $pStatus = $kStatus = null;

        if ($sample->isAnalyzed()) {
            $fertRec = $this->fertilizer->recommend(
                $sample->ph_level !== null ? (float) $sample->ph_level : null,
                (float)$sample->nitrogen_level,
                (float)$sample->phosphorus_level,
                (float)$sample->potassium_level
            );
            $nStatus = $this->fertilizer->getNutrientStatus('nitrogen',   (float)$sample->nitrogen_level);
            $pStatus = $this->fertilizer->getNutrientStatus('phosphorus', (float)$sample->phosphorus_level);
            $kStatus = $this->fertilizer->getNutrientStatus('potassium',  (float)$sample->potassium_level);
        }

        $riceCrop       = Crop::where('name', 'Rice')->first();
        $scheduleMatrix = $riceCrop ? CropFertilizerSchedule::matrixForCrop($riceCrop->id) : [];
        $fertTypes      = CropFertilizerSchedule::fertilizerTypes();

        $html     = view('samples.excel-export', compact(
            'sample', 'fertRec', 'nStatus', 'pStatus', 'kStatus',
            'scheduleMatrix', 'fertTypes'
        ))->render();

        $filename = 'soil_sample_' . $sample->id . '_' . now()->format('Ymd') . '.xls';
